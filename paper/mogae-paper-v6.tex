...
\textbf{Unique to Grove?} Yes. The gate simultaneously controls three
resources with one learned signal: (1)~expert routing, (2)~forward
precision, (3)~backward precision. Standard quantized models use
uniform precision. MoE models route but do not modulate precision.
Only the gated architecture enables per-layer, per-token, dynamic
precision allocation.

\textbf{Prediction.} With INT4-only layers at 50\% of the model,
effective memory bandwidth drops by 25\%, yielding a corresponding
throughput increase on bandwidth-bound hardware (Blackwell: 1792~GB/s).
Combined with layer skipping and bridges, predicted throughput exceeds
350~tok/s.
