...
\subsection{Why MLP layers, not attention?}
\label{sec:pilot-mlp}

Our adapters inject into the MLP (feed-forward) sublayers only, leaving
attention unchanged. This follows Geva et al.~\cite{geva2021transformer}
who showed that transformer FFN layers act as key-value memories for
factual knowledge. In our pilots, injecting into MLP layers at
positions 12--35 (upper half) captured domain knowledge while leaving
identity/style processing (layers 0--11) untouched. Attention layers
encode relational structure that we found is better left frozen.
...
Combining all architecture-enabled optimizations on Qwen3-8B
(36~layers, RTX~PRO~6000 Blackwell):

\begin{table}[h]
\centering
\caption{Predicted inference efficiency of the combined hierarchical
scheme. All predictions assume single-GPU, single-stream decode at
batch size~1.}
\label{tab:perf-prediction}
\begin{tabular}{@{}lrrl@{}}
\toprule
Component & Layers & Bandwidth & Status \\
\midrule
Skip (passthrough) & 20 & 0 & Validated \\
Bridge surrogates & 4--6 & ${\sim}8$~MB & Validated (single) \\
Active (differential FP4/FP8) & 10--12 & ${\sim}1.9$~GB & Measured \\
\midrule
Total weight reads & & ${\sim}1.9$~GB & \\
At 1792~GB/s & & ${\sim}1.1$~ms & \\
+ overhead (attention, norms) & & ${\sim}1.7$~ms & \\
\midrule
\textbf{Predicted decode time} & & ${\sim}2.8$~ms & \\
\textbf{Predicted throughput} & & \textbf{${\sim}350$~tok/s} & \\
\bottomrule
\end{tabular}
\end{table}

Current validated performance: 228~tok/s (FP8 uniform, 20-layer skip,
CUDA graph). The predicted ${\sim}350$~tok/s represents a further
$1.5\times$ improvement from differential quantization and bridge
surrogates alone, without requiring native FP4 tensor core kernels.
With native NVFP4 CUTLASS kernels (compiled but not yet integrated
into our pipeline), the target rises to $400{+}$~tok/s.
