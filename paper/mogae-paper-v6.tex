...
\textbf{Mechanism.} Each weight is stored as two packed INT4
registers occupying 8~bits total:
\begin{itemize}[nosep]
\item \textbf{Register A} (low 4~bits): coarse approximation of the weight.
\item \textbf{Register B} (high 4~bits): residual that, combined with A,
      recovers INT8 precision.
\end{itemize}

The gate controls which registers are read per layer:
\begin{equation}
\label{eq:variable-precision}
W_{\text{eff}} = \begin{cases}
\text{dequant}(A) & \text{if } \sigma(g) < \tau \text{ (INT4, half bandwidth)} \\
\text{dequant}(A, B) & \text{if } \sigma(g) \geq \tau \text{ (INT8, full precision)}
\end{cases}
\end{equation}

Memory footprint is constant (8~bits per weight, same as INT8).
Memory \emph{bandwidth} scales with the gate: low-gate layers read
4~bits per weight (register~A only), high-gate layers read 8~bits
(both registers). On bandwidth-bound hardware, this halves the cost
of non-critical layers.

\textbf{Training and inference share the same code path.} The forward
pass is identical: read INT4 or INT8 per layer based on the gate. For
training, the backward pass follows the same precision allocation---
low-gate layers receive approximate gradients (INT4), high-gate layers
receive precise gradients (INT8). LoRA adapter parameters are always in
BF16. The base model weights never change; only the adapter is updated.
This means training imposes \emph{zero overhead} on the inference path.
...
\midrule
$\approx 0$ & INT4 & Bridge (0.5M FLOPs) & 4 bits/weight \\
$\approx 0.5$ & INT8 & Full layer (300M FLOPs) & 8 bits/weight \\
$\approx 1$ & INT8 & Full layer + adapter & 8 bits/weight \\
\bottomrule
\end{tabular}
\end{center}
...
weights are always in INT4-packed format. No mode switch between training
and inference. Training adds only the backward pass for adapter parameters;
the forward pass is identical.

\textbf{Validated.} INT8 (both registers) produces coherent output
