...
signal for \emph{when} to fork (speculative splitting with rollback), \emph{where} to fork
...
\emph{speculative splitting with rollback} provides a data-driven
alternative, accepting only 2 of 8 attempted splits and reverting the rest.
...
10 independently-trained adapters compose via joint gate fine-tuning with
94\% of composition checks passing.
...
\end{equation}
where $e_i$ is expert $i$ and $t$ is the predicted token. \textbf{We do
not claim a new signal.} Learntropy in our current implementation is
per-expert cross-entropy loss. The term was independently coined by
Wozniak~\cite{wozniak2018learntropy} to denote the attractiveness of a
signal to the learn drive system---a net reward that depends on prior
knowledge and can go negative when material exceeds the learner's
capacity. Wozniak's original learntropy is \emph{learner-relative}:
surprise relative to what the model already knows. Our Hypothesis~1
operationalization ($\mathcal{L}_{\text{expert}} - \mathcal{L}_{\text{base}}$)
measures this and shows correlation $r = 0.64$ with raw CE---they are
not equivalent; the learner-relative formulation captures residual
difficulty beyond the base model's capacity.

What we claim is that \emph{one existing signal} (loss) drives
\emph{three decisions} that are normally handled separately: learning
rate modulation (Section~\ref{sec:analysis}), split timing
(Section~\ref{sec:speculative-split}), and training duration
(Section~\ref{sec:delta-gating}). The \emph{unification} is the
contribution, not the signal itself. In particular, learning speed
($-d/dt[\text{EMA}(\mathcal{L})]$) detects the Goldilocks zone---the
productive training window---and overfitting onset, providing a
principled early-stopping criterion that does not require a held-out
validation set.

A unique property of the hot-pluggable adapter architecture is that the
frozen base model is always available as a \emph{reference point}. This
enables a \emph{relative} learntropy formulation:
$\mathcal{L}_{\text{rel}}(t) = \mathcal{L}_{\text{expert}}(t) -
\mathcal{L}_{\text{base}}(t)$, measuring not absolute difficulty but
how much the adapter helps or hurts on each token. In standard
fine-tuning, the base weights are overwritten---no reference exists.
In our architecture, relative learntropy is always computable and
connects directly to Piaget: tokens where $\mathcal{L}_{\text{rel}} < 0$
are in the accommodation zone (the adapter provides knowledge the trunk
lacks); tokens where $\mathcal{L}_{\text{rel}} \approx 0$ are in the
assimilation zone (the trunk suffices); tokens where
$\mathcal{L}_{\text{rel}} > 0$ represent negative learntropy in
Wozniak's sense (the adapter \emph{harms} performance---the learning
attempt should be suppressed).
% TODO: Replace raw CE with relative learntropy for LR modulation,
% split timing, and curriculum weighting in 8B experiments.
% Pre-registered: relative-learntropy-8b-grove.md
...
Our original hypothesis was that bimodal learntropy distributions would
signal when an expert needs to split: some tokens easy, others hard,
indicating the expert serves two populations. In practice, bimodality
detection (Hartigan's dip test, kurtosis) never fires on generic C4
data---the loss distributions are unimodal with heavy tails, not cleanly
bimodal. We therefore replaced bimodality-triggered splitting with
\emph{speculative splitting with rollback}
(Section~\ref{sec:speculative-split}): attempt a split on schedule,
evaluate for $N$ steps, and revert if it did not help. This inverts
the problem---instead of predicting \emph{when} to split, the system
tries and evaluates. The bimodal framing remains theoretically
motivated but is not the operational mechanism.

When an expert's capacity is insufficient, it must either adapt or divide. Following biological cells, we distinguish two responses:
...
\textbf{Mitosis (expert split).} If rank increase is insufficient, the expert undergoes \emph{mitosis}: it divides into two daughter experts, each inheriting the parent's weights plus a small random perturbation. Like biological daughter cells, they start genetically identical but \emph{differentiate} through exposure to different environments (data partitions) and explicit pressure to diverge (contrastive loss). The operational procedure is \emph{speculative splitting with rollback}:
\begin{enumerate}
    \item Identify the split candidate: the expert with the highest learntropy (most struggling).
    \item Checkpoint the current state.
    \item Duplicate the LoRA adapter with small perturbation.
    \item Train both daughters for an evaluation window ($\sim$500 steps).
    \item If loss improves: accept the split. If not: rollback to checkpoint.
    \item Daughters \emph{differentiate} through their data environment and contrastive loss (Section~\ref{sec:contrastive}).
\end{enumerate}
In a 25K-step experiment, the system accepted 2 out of 8 attempted splits (at steps 9K and 21K), growing to 4 experts with all active ($M_{ij}$ CV 1.52). Early splits (3K, 6K) were correctly rejected as premature (Section~\ref{sec:speculative-split}).
...
\paragraph{Fine-tuning degradation.}
LoRA fine-tuning is known to degrade performance on tasks outside the
fine-tuning domain---a manifestation of catastrophic
forgetting~\cite{biderman2024lora}. Standard mitigations include
regularization (EWC, L2-SP), replay buffers, or careful learning rate
schedules. We observe $+25$--$32\%$ generic PPL degradation with
standard PEFT LoRA and propose per-layer gating as a lightweight
alternative: a learned gate that activates the adapter selectively,
reducing degradation to $+2.9\%$ (output-level) or achieving net
improvement ($-6\%$ to $-13\%$, per-layer) without requiring
regularization or replay.
...
\paragraph{Multi-adapter ecosystems.}
HuggingFace PEFT~\cite{peft2023} provides LoRA loading, composition (stack,
fuse), and layer selection via \texttt{layers\_to\_transform}. However, PEFT
applies adapters \emph{uniformly} across selected layers with no per-layer
gating, no learned routing between adapters, and no domain-selective
activation. S-LoRA~\cite{slora2023} and vLLM enable efficient multi-LoRA
\emph{serving} but address deployment, not training-time selectivity.
MixLoRA~\cite{mixlora2024} learns per-factor routing but not per-layer
delta gating. Our per-layer sigmoid gate---initialized near OFF
($\sigma(-2) \approx 0.12$) and trained to activate selectively---is
distinct from all of these.

\paragraph{Dynamic expert growth.}
DynMoE~\cite{dynmoe2025} (ICLR 2025) adds new experts when tokens fail to
activate existing ones (activation gap trigger). VMoER~\cite{vmoer2025} and
MoGU~\cite{mogu2025} use uncertainty for routing \emph{selection} but not
for expert \emph{creation}. Our approach differs in three ways: (1) we
\emph{split} an existing expert rather than adding a blank one, preserving
learned weights; (2) the trigger is per-expert learntropy (uncertainty about
what to learn), not activation gaps; (3) speculative splitting with rollback
treats growth as a reversible transaction---the system attempts a split,
evaluates for $N$ steps, and reverts if loss does not improve. To our
knowledge, expert splitting and speculative rollback have no precedent in
the MoE literature.

\paragraph{Selective training and compression progress.}
Rho-1~\cite{rho1_2024} shows that scoring tokens by
$\mathcal{L}_{\text{train}} - \mathcal{L}_{\text{reference}}$ and training
only on high-scoring tokens achieves 30\% improvement with 3\% of pretraining
data on math tasks. Our architecture is uniquely suited to this: the frozen
base model IS the reference model, always available for comparison. In standard
fine-tuning, the reference is destroyed when weights are overwritten. In our
framework, $\mathcal{L}_{\text{expert}}(t) - \mathcal{L}_{\text{base}}(t)$
is computed for free during every forward pass---no additional model needed.
Schmidhuber's \emph{compression progress}~\cite{schmidhuber2008} further
refines this: the reward is not current prediction error but its
\emph{first derivative}---how fast the model's predictions improve. Our
learning speed signal ($-d/dt[\text{EMA}(\mathcal{L})]$, Section~\ref{sec:delta-gating})
implements this directly and identifies the productive training window
(Goldilocks zone) where compression progress is positive.

\paragraph{What is novel in this work.}
We do not claim novelty for LoRA adapters, frozen trunks, or multi-adapter
serving---these are established. Our contributions are:
\begin{enumerate}[nosep]
\item \textbf{Per-layer delta gates} that learn selective activation from
      data (not uniform application as in standard PEFT), providing
      domain selectivity and generic-output protection simultaneously.
\item \textbf{Expert splitting with speculative rollback}: growing experts
      by dividing existing ones, triggered by learntropy, with automatic
      revert on failure.
\item \textbf{Emergent IDK and cache miss detection}: the trained gate
      naturally discriminates in-domain from out-of-domain text
      (2.25--4.11$\times$ ratio); at grove scale, 81\% of uncovered
      topics are correctly detected as cache misses.
\item \textbf{Joint gate composition}: independently trained adapters
      compose without interference via joint gate fine-tuning with softmax
      normalization (95/101 checks at 10 adapters), enabling distributed
      community training.
\item \textbf{Ensemble ablation inference}: selectively suppressing
      adapters at inference time to identify adapter-dependent claims,
      enabling epistemic verification unique to modular architectures.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Split timing relies on speculative rollback, not prediction.}
          Our original hypothesis was that bimodal learntropy distributions
          would signal when to split. In practice, bimodality detection
          (Hartigan's dip test, kurtosis) never fires on generic C4 data.
          Three candidate signals (ILV, router margin bimodality, learntropy
          slope) correctly predict \emph{which} expert should split but not
          \emph{when}. The operational solution is speculative splitting with
          rollback (Section~\ref{sec:speculative-split}): attempt a split on
          schedule, evaluate, revert if unhelpful. This works (2/8 accepted,
          6 rolled back) but is reactive rather than predictive. A principled
          split trigger remains an open problem---relative learntropy
          ($\mathcal{L}_{\text{expert}} - \mathcal{L}_{\text{base}}$) is a
          candidate but has not yet been tested for this purpose.
    \item \textbf{Framework tested on one model family only.} All experiments use
          ...
          demonstrated.
    \item \textbf{Learntropy definition is honest but limited.} Our current
          learntropy is per-expert cross-entropy loss---a standard signal used
          in a unified way. The learner-relative formulation
          ($\mathcal{L}_{\text{expert}} - \mathcal{L}_{\text{base}}$, correlation
          $r = 0.64$ with raw CE) and learning speed
          ($-d/dt[\text{EMA}(\mathcal{L})]$) provide richer variants that we
          use for training duration decisions but have not yet integrated into
          split timing. A principled learntropy function that models net
          learning value---closer to Wozniak's~\cite{wozniak2018learntropy}
          definition---may improve curriculum-driven forking.
    \item \textbf{Hallucination risk remains.} The gate-based ``I don't know''
          detection (Section~\ref{sec:delta-gating}) distinguishes in-domain from
          out-of-domain text (2.25--4.11$\times$ ratio for high-selectivity
          adapters), but this is \emph{domain membership}, not \emph{factual
          correctness}. An adapter can confidently generate domain-appropriate
          but factually wrong text. The long tail of crystallized knowledge that
          community-contributed adapters aim to cover should reduce the frequency
          of hallucination (the model hallucinates less when it has actual
          knowledge), but it does not eliminate confident errors within covered
          domains. Detecting and preventing within-domain hallucination remains
          an open problem that requires mechanisms beyond gate activation.
\end{enumerate}
...
\paragraph{Speculative splitting with rollback: data-driven growth.}
\label{sec:speculative-split}
A learntropy-driven split trigger remains elusive (bimodality detection
never fires on generic C4 data). We therefore tested an alternative:
\emph{speculative splitting with rollback}. Every 3,000 steps, the system
attempts to split the highest-learntropy expert, trains for a 500-step
evaluation window, and \emph{reverts} if the split did not reduce loss.
In a 25K-step experiment on Qwen3-8B, the system attempted 8 splits and
\textbf{kept only 2} (at steps 9K and 21K), rolling back the other 6.
Early splits (3K, 6K) were correctly rejected as premature; late splits
(12K, 15K, 18K, 24K) were rejected because the existing experts had
already partitioned the learnable signal. The result: 4 experts with all
4 active ($M_{ij}$ CV 1.52). The cost is at most 500 steps per failed
attempt---a modest overhead that eliminates the need for any timing
signal. This inverts the split-timing problem: instead of predicting
\emph{when} to split, the system tries splitting and evaluates whether it
helped. To our knowledge, speculative expert splitting with automatic
rollback has no precedent in the MoE literature.
...
\paragraph{Per-layer gates as Piagetian assimilation/accommodation.}
The gate magnitude at each layer implements a continuous
assimilation/accommodation spectrum in Piaget's
sense~\cite{piaget1952}. A low gate ($g_l \approx 0.1$) means the
trunk's existing representation suffices---the adapter provides a
small correction within the trunk's existing schema
(\emph{assimilation}). A high gate ($g_l \approx 0.9$) means the trunk
lacks the representational capacity for this domain at this layer---the
adapter must override it (\emph{accommodation}). The paper already
identifies this distinction at the rank level (low rank = assimilation,
high rank = accommodation, Section~\ref{sec:method}). The per-layer
gate extends it: within a single adapter, different layers operate in
different Piagetian regimes simultaneously. Layer~35 ($g = 0.99$) fully
accommodates; layer~30 ($g = 0.13$) assimilates. The model learns
\emph{where} accommodation is needed, not just \emph{whether}.
% TODO: Validate empirically --- correlate per-layer gate value with
% per-layer learntropy (token difficulty at that layer). If high-gate
% layers are also high-learntropy layers, the Piaget link is confirmed:
% the model accommodates precisely where its existing knowledge fails.
...
\paragraph{Learntropy-guided training recipe.}
Raw cross-entropy loss serves as the current learntropy implementation.
Used as learning speed ($-d/dt[\text{EMA}(\mathcal{L})]$), it identifies
the \emph{Goldilocks zone}: the productive training window where the
model is learning domain structure rather than memorizing or plateauing.
Combined with per-layer gating, the optimal recipe is: learntropy-guided
early stop at $\sim$500 adapter-training steps, followed by 1,500 steps
of gate-only training. This matches ungated performance on domain PPL
($-22\%$ to $-32\%$) while \emph{improving} generic PPL ($-12\%$ to
$-13\%$ vs.\ base), compared to longer ungated training ($-8.7\%$ to
$-12.6\%$ generic improvement) or longer gated training ($-4.8\%$ to
$-7.5\%$).

\begin{center}
\begin{tabular}{@{}lrr@{}}
\toprule
Configuration & Domain PPL & Generic PPL \\
\midrule
Ungated 500 steps & $-22\%$ to $-32\%$ & $-8.7\%$ to $-12.6\%$ \\
Gated 2000+1500 & $-8\%$ to $-20\%$ & $-4.8\%$ to $-7.5\%$ \\
\textbf{Gated 500+1500} & $\mathbf{-22\%}$ \textbf{to} $\mathbf{-32\%}$ & $\mathbf{-12\%}$ \textbf{to} $\mathbf{-13\%}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The mechanism: longer adapter training overfits to domain data,
which the gate must then compensate for. Stopping adapter training at the
learntropy-detected productive window preserves generality while
capturing domain structure. Replicated on 2 domains (BBC news, Dutch
cuisine). \emph{(Class~B: replicated, 2 domains.)}
...
\paragraph{Gate as ``I don't know'' detector.}
A domain adapter's gate naturally discriminates between in-domain, out-of-domain,
and generic text. The BBC news adapter produces gate activation of 0.783 on
in-domain text but only 0.348 on mixed unknown topics (physics, programming,
medical, sports), 0.305 on cuisine text (a second unknown domain), and 0.302 on
generic C4. All three out-of-domain types yield gate $<0.40$, with known/unknown
ratios of 2.25--2.59$\times$---substantially stronger than the 1.25$\times$
learntropy gap reported earlier. This suggests that ``I don't know'' detection
requires no additional signal: the existing gate \emph{is} the detector.
However, the effect is adapter-dependent: the cuisine adapter shows weaker
discrimination (1.86$\times$), likely because food-related text overlaps more
with generic language than news does.

\paragraph{Cache miss detection at grove scale.}
Beyond per-adapter IDK detection, the grove's gate activations enable
\emph{cache miss detection}: identifying queries that fall outside
\emph{all} loaded adapters' domains. We tested 32 queries spanning 16
covered topics and 16 uncovered topics (law, economics, history, art,
music, geography, psychology, philosophy). Of the 16 uncovered queries,
13 were correctly detected as cache misses (max gate $<0.30$), yielding
an 81\% detection rate with only 6\% false coverage (1/16 uncovered
queries falsely classified as covered). This enables a demand-driven
training pipeline: logged cache misses reveal which topics users ask
about but no adapter covers, directing training priorities toward actual
demand rather than guesswork. (Pre-registered, single run.)

\paragraph{Multi-adapter composition requires joint gate training.}
When two adapters (BBC news + Dutch cuisine) are loaded simultaneously with
independently-trained gates, the cuisine gate fires at 0.49 on BBC text,
adding interference ($+5.9\%$ PPL vs.\ single adapter). Softmax normalization
reduces leakage ($0.49 \to 0.18$) but dampens the primary gate ($0.79 \to 0.66$).
\textbf{Joint gate fine-tuning}---freezing both adapters and training only the
gates on mixed-domain data---resolves this completely: cross-gate leakage drops
to 0.034 (BBC$\to$cuisine) and 0.011 (cuisine$\to$BBC), primary gates reach
0.82 and 0.85, and both domain PPLs improve $>$50\% vs.\ base. Generic PPL also
improves ($-8.2\%$). The key insight: gates trained in isolation learn ``my domain
vs.\ generic'' but not ``my domain vs.\ other adapters' domains.'' Joint training
teaches the full discrimination.

\paragraph{10-adapter grove composition.}
Scaling from 3 to 10 independently-trained adapters (BBC news, Dutch
cuisine, Wing Chun, physics, biology, chemistry, mathematics, computer
science, medicine, AI/ML) with joint gate fine-tuning, 95 of 101
composition checks pass (94\%). The 6 failures are semantically correct
overlaps between related domains (physics$\leftrightarrow$chemistry,
biology$\leftrightarrow$medicine, mathematics$\leftrightarrow$computer
science---cross-leakage 0.11--0.16 where the threshold is 0.10).
Generic perplexity \emph{improves} by $-14.2\%$ compared to the base
model. This demonstrates that the grove architecture scales to
double-digit adapter counts without interference or generic degradation.

\paragraph{Standard benchmarks improve with grove adapters.}
A 3-adapter grove (BBC news, Dutch cuisine, Wing Chun martial arts---trained
independently, composed via joint gate fine-tuning) improves standard
reasoning benchmarks compared to the unmodified base model:
ARC-Challenge acc\_norm $56.3\% \to 57.5\%$ ($+1.2\%$),
HellaSwag acc\_norm $75.0\% \to 77.5\%$ ($+2.6\%$).
A replication with joint-trained gates (which produce cleaner routing)
yields slightly lower but still positive improvements: ARC $+1.0\%$,
HellaSwag $+2.1\%$. The small gap suggests that routing quality and
benchmark performance are partially independent---both should be
reported. The improvement is notable because the adapters were trained
on domain-specific text, not on benchmark tasks. The gated adapters act
as soft regularizers, providing complementary representations that
benefit even out-of-domain reasoning. (Single run, 0-shot, not yet
replicated.)

\paragraph{Sparse routing eliminates the cost of inactive adapters.}
With 10 adapters loaded, dense routing (computing all adapter forward passes
at every layer) reduces throughput from 60.8 to 12.5~tokens/s ($-79\%$). A
hard gate threshold ($<0.05 \to 0$, skip adapter computation entirely)
recovers much of this: sparse routing achieves 22.1~tokens/s, a $+77\%$
speedup over dense routing. On average, 66\% of adapter computations are
skipped because their gates are below threshold. Prefill throughput improves
similarly ($+72\%$, 2337 vs.\ 1361~tokens/s). This is the same mechanism
used by Mixture-of-Experts models (top-$k$ routing): only compute the experts
that the router selects. At scale (hundreds of adapters), sparse routing is
essential---the cost grows with the number of \emph{active} adapters per
token, not the total number in the grove.

\paragraph{Ensemble ablation inference.}
The grove's modular structure enables a novel verification mechanism:
\emph{ensemble ablation inference}. For a given query, the system
identifies which adapters are active (gate $>0.05$), then re-runs
inference with each active adapter individually suppressed. Where
answers change, the claim is adapter-dependent and may warrant
verification. In testing on 8 diverse factual questions, routing
correctly identifies 1--3 active adapters per question (physics for
Heisenberg uncertainty, computer science for compiler optimization,
medicine for antibiotic resistance). This capability is unique to
modular architectures: monolithic models cannot selectively disable
knowledge domains, and the shared trunk means $N$ ablation runs cost
approximately $N$ forward passes rather than $N$ full model evaluations.
(Pre-registered, single run; consensus accuracy not yet measured.)

\paragraph{Medical benchmark: an honest negative about data quality.}
\label{sec:medical-negative}
To test whether domain adapters add factual knowledge or merely style,
we evaluated a medicine adapter (trained on 500 FineWeb-Edu texts
filtered by medical keywords) on 6 standard medical benchmarks via
lm-eval-harness. The adapter \textbf{degrades all 6 benchmarks}:
MedQA $64.1\% \to 60.1\%$ ($-4.0$pp),
MMLU Professional Medicine $82.0\% \to 71.3\%$ ($-10.7$pp),
MMLU Anatomy $70.4\% \to 68.9\%$ ($-1.5$pp),
MMLU Clinical Knowledge $79.6\% \to 78.1\%$ ($-1.5$pp),
MMLU College Medicine $75.1\% \to 74.0\%$ ($-1.2$pp),
MMLU Medical Genetics $82.0\% = 82.0\%$ (neutral).
The per-layer gate protects generic perplexity ($17.5 \to 16.9$, an
improvement) but cannot compensate for low-quality training data. This
establishes that \textbf{data quality, not architecture, is the
bottleneck for knowledge injection}. The gate mechanism works as
designed---it selectively applies the adapter---but when the adapter
encodes noise rather than knowledge, selective application of noise
still degrades performance. Domain adapters require curated,
high-quality training data; keyword filtering of general corpora is
insufficient. (Pre-registered, single run.)

\paragraph{Uniform vs.\ per-layer gated LoRA.}
To test whether per-layer gating adds value over standard uniform LoRA
(the key differentiator between our approach and standard PEFT), we
trained both on the same medical data with identical rank and steps.
Result: per-layer gating \textbf{protects generic perplexity} (17.5
$\to$ 16.9 vs.\ 17.5 $\to$ 38.6 for uniform), but neither adds
medical knowledge with FineWeb-filtered data. The gate's value is
\emph{protection}---preventing the adapter from degrading the base
model on out-of-domain text---not knowledge injection. Uniform LoRA
catastrophically degrades generic performance ($+120\%$ PPL) because
every layer applies the domain update indiscriminately. This validates
the per-layer gate as essential for hot-pluggable deployment, even when
the adapter itself provides no knowledge benefit.

\paragraph{Per-layer gating protects against standard LoRA degradation.}
The generic-PPL degradation observed above is not specific to our custom
Expert class---it affects standard HuggingFace PEFT LoRA equally.
Ungated LoRA fine-tuning (standard PEFT, identical rank and data)
degrades generic perplexity by $+25$--$32\%$. Our per-layer gate reduces
this to $+2.9\%$ at the output level, and per-layer analysis shows
$-6\%$ to $-13\%$ improvement on individual layers---meaning the gated
adapter \emph{improves} the base model's representations at most layers
while concentrating domain knowledge where it is needed. This positions
per-layer gating as a general-purpose fix for LoRA fine-tuning
degradation, independent of our Expert architecture.
\emph{(Replicated on 2 domains: BBC news, Dutch cuisine; Class~B.)}

\paragraph{Top-$k$ throughput scaling.}
\label{sec:topk-throughput}
Sparse routing (threshold-based gate skipping) scales smoothly with the
number of active adapters. With 10 adapters loaded on Qwen3-8B:
...
\midrule
Base (no adapters) & 60.8 & 100\% \\
Top-1 (1 active) & 38.2 & 90\% \\
Top-2 (2 active) & 31.2 & 80\% \\
Top-3 (3 active) & 26.2 & 70\% \\
Dense (all 10) & 12.5 & 0\% \\
\bottomrule
\end{tabular}
\end{center}

Each additional active adapter costs $\sim$4~tok/s, a linear overhead.
At top-1, throughput is 63\% of the base model; at top-3, still 43\%.
Dense routing ($-79\%$) is unacceptable at scale; sparse routing makes
the cost proportional to the number of \emph{relevant} adapters, not
the total number in the grove.

% ============================================================================
\section{Conclusion}

We set out to separate fluid intelligence from crystallized knowledge using
variable-rank LoRA expert adapters. The results establish six principal
findings, several deployment-ready capabilities, and one important honest
negative, all driven by a unifying framework centered on learntropy.
...
\textbf{Deployment capabilities.} The grove scales to 10 independently-trained
adapters with 94\% composition checks passing and $-14.2\%$ generic PPL
improvement. Sparse routing recovers $+77\%$ throughput over dense routing by
skipping inactive adapters. Cache miss detection identifies 81\% of
uncovered topics, enabling demand-driven growth. Ensemble ablation
inference---selectively suppressing adapters to detect adapter-dependent
claims---provides epistemic verification unique to modular architectures.

\textbf{The frozen base as epistemic reference.} A property unique to
hot-pluggable adapter architectures: the base model is always available
for comparison. This enables \emph{relative learntropy}
($\mathcal{L}_{\text{expert}} - \mathcal{L}_{\text{base}}$), measuring
not absolute token difficulty but how much the adapter helps or hurts---a
learner-relative signal in Wozniak's sense. Standard fine-tuning
overwrites the base weights, destroying this reference. In our
architecture, the Piagetian regime of each layer is directly measurable:
assimilation (adapter $\approx$ base), accommodation (adapter $\ll$ base),
or interference (adapter $>$ base, Wozniak's negative learntropy).
% TODO: Report Piaget gate--learntropy correlation when experiment completes.
% TODO: Report 8B grove results with relative learntropy for LR/split/curriculum.

\textbf{Honest negative: data quality bottleneck.} A medicine adapter trained
on keyword-filtered FineWeb-Edu data degrades all 6 medical benchmarks
(MedQA $-4.0$pp, MMLU Professional Medicine $-10.7$pp). The per-layer gate
protects generic PPL but cannot compensate for low-quality training data.
Architecture is not the bottleneck for knowledge injection---data quality is.
Per-layer gating remains essential: without it, uniform LoRA catastrophically
degrades generic performance ($+120\%$ PPL).
...
\item \textbf{When to fork}: the highest-learntropy expert is the split
      candidate. Speculative splitting with rollback evaluates whether the
      split actually helps---accepting 2/8 attempts in practice.
