...
Bimodality detection (below) partially recovers
Wozniak's distinction between productive and unproductive difficulty
by identifying when an expert's loss distribution contains both
learnable and unlearnable populations.
...
\begin{enumerate}
    ...
    \item \textbf{Split timing is scheduled, not learntropy-driven.}
          ...
          The scheduled approach works well in practice, but
          \emph{speculative splitting with rollback} provides a
          data-driven alternative that eliminates the need for any
          timing signal (see Section~\ref{sec:speculative-split}).
