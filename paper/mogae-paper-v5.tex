...
Across 25+ experiments on Qwen3-1.7B and Qwen3-8B, we establish three
findings. First, \emph{causal locality}---selective ablation damage, not
weight orthogonality---is the correct metric for modularity, and the
prerequisite for compute efficiency. Second, causal locality requires
sufficient model scale: $M_{ij}$ CV plateaus at 0.16 for 1.7B across all
configurations tested, but reaches 0.50--0.67 at 8B, with expert removal
damage varying 3.5--13.5$\times$ across domains. The 8B expert-augmented
model reduces perplexity by 19\% compared to the base. Third, different
knowledge types require different \emph{fork depths}: layer divergence
analysis reveals a U-shaped profile where early layers (0--4) and late
layers (24+) show high domain-discriminative fisher ratios, while middle
layers (8--20) converge---suggesting that code should fork early
(different computational substrate) while medical/legal knowledge shares
the reasoning trunk and forks late (different vocabulary only).

This motivates a \emph{forest} rather than a single tree: each knowledge
type grows its own branching structure, with learntropy driving all
adaptation decisions. We outline a path from rule-based controllers to
learned training policies (Bayesian optimization over the
learntropy-driven decision space).
\end{abstract}
...
mechanism) increases the $M_{ij}$ CV from 0.68 to 1.09---a 61\% improvement.
Both experts remain active, with domain damage following the same pattern as
all previous experiments: conversational most vulnerable (+1,158 $\Delta$PPL
when Expert~0 removed), medical least (+380). This confirms that the
2-expert equilibrium at rank-4 is not a fundamental limit of the framework
but a consequence of operating in too few dimensions. More rank = more
signal available for domain-selective learning.
