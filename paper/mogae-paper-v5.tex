...
and generic text. When the BBC news adapter encounters topics it was not trained
on (physics, programming, medical, sports), the mean gate activation is 0.348---
nearly identical to generic C4 text (0.302) and well below in-domain activation
(0.783). The known/unknown ratio of 2.25$\times$ is substantially stronger than
the 1.25$\times$ learntropy gap reported earlier, because the gate is trained
specifically to discriminate its domain from non-domain. This suggests that
``I don't know'' detection requires no additional signal: the existing gate
\emph{is} the detector. (Single observation, 1~adapter---not yet replicated.)
