...
independent gates, the cuisine gate fires at 0.49 on BBC text, adding
interference ($+5.9\%$ and $+6.8\%$ PPL vs.\ single adapter). Softmax
normalization across gates reduces cross-gate leakage (cuisine gate on BBC:
$0.49 \to 0.18$) and cuisine PPL cost ($+6.8\% \to +1.8\%$), but dampens
the primary gate ($0.79 \to 0.66$) and leaves BBC PPL at $+4.7\%$.
Normalization is a significant improvement but does not fully resolve the
composition problem. Joint gate training or a learned router may be needed
for clean multi-adapter deployment.
