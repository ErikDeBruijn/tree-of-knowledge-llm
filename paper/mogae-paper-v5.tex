...
\item \textbf{Expert splitting with speculative rollback}: growing experts
      ...
      revert on failure.
\item \textbf{Emergent IDK detection}: the trained gate naturally
      discriminates in-domain from out-of-domain text (2.25--4.11$\times$
      ratio), requiring no additional module.
\item \textbf{Joint gate composition}: independently trained adapters
      ...
      normalization, enabling distributed community training.
\end{enumerate}
...
This is notable because the adapters were trained on domain-specific text,
not on benchmark tasks. The improvement likely reflects the same mechanism
as the generic PPL improvement: the delta-gated adapters act as soft
regularizers, providing complementary representations that benefit even
out-of-domain reasoning. (Single run, 0-shot, not yet replicated.)
