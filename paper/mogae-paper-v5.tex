...
Across 35+ experiments on Qwen3-1.7B and Qwen3-8B, we establish four
findings. First, \emph{causal locality}---selective ablation damage, not
weight orthogonality---is the correct metric for modularity. Second, causal
locality requires sufficient model scale: $M_{ij}$ CV plateaus at 0.16 for
1.7B but reaches 0.50--0.67 at 8B, with the expert-augmented model reducing
perplexity by 19\%. Third, learntropy-modulated per-expert learning rates
break the dead-expert problem: slow-learning experts (assimilation mode)
crystallize into the most critical components, while over-accommodating
experts go dormant---a Piagetian inversion validated across multiple
configurations. Fourth, \emph{hierarchical bifurcation} (1$\to$2$\to$4,
progressive tree branching) activates all 4 experts for the first time,
where flat 1$\to$4 routing consistently produces dead experts regardless of
rank or learning rate. The tree structure enforces differentiation at each
level: functional at layer~18, domain at layer~24.

Different knowledge types require different override depths---code diverges
early, medical/legal share the reasoning trunk and diverge late---motivating
a \emph{forest} of variable-depth specialization profiles. We outline a path
from rule-based learntropy controllers to learned training policies.
\end{abstract}
...
However, this relationship is \textbf{non-monotonic}. At rank-1024
(25\% of hidden dimension, ``HiRA'' regime), the $M_{ij}$ CV
\emph{drops} to 0.47---lower than rank-4 (1.07). Both experts remain
active and important, but domain selectivity vanishes. The massive
subspace gives each expert enough capacity to learn everything, eliminating
the bottleneck that forces specialization. This reveals a fundamental
trade-off: \emph{low rank forces specialization through information
bottleneck; high rank enables capacity but removes differentiation
pressure}. The optimal rank is not the highest but the one that balances
capacity against specialization pressure---empirically, rank-8 at the 8B
scale.
...
variable-rank LoRA expert adapters. The results establish three principal
findings and point to a unifying framework.
