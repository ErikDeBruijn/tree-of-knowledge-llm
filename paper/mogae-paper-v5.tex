...
\item \textbf{When to abstain}: at inference time, high learntropy on a
      query indicates the model lacks relevant knowledge. Instead of
      hallucinating, the system can report uncertainty and optionally
      discover a verified adapter from a registry via query-embedding
      similarity. This reframes hallucination from an alignment problem
      to a routing problem: if no adapter reduces learntropy, say
      ``I don't know.''
\end{enumerate}
