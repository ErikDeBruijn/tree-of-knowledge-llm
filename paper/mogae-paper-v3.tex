...
decisions—they are one. A rank-1 adapter is a \emph{lens}: a 4\,KB correction
to the generalist, always cached. A rank-64 adapter is a \emph{crystal}: a
256\,KB specialist that reshapes computation fundamentally, loaded from NVMe on
demand. The rank is not a hyperparameter to tune—it is a \emph{measurement} of
how different this knowledge is from the core.

\placeholder{Figure~1: The \tok{} tree. A Qwen3-1.7B trunk (layers 1–14) feeds
into branching LoRA adapters at increasing depth. Each node shows its adapter
rank (r=1 at first fork, growing to r=16..64 at leaves). Annotations indicate
storage tier: VRAM-resident for the trunk and low-rank adapters, NVMe for
high-rank specialists. The reader should understand the entire architecture
...
An expert splits when the \emph{distribution} of learntropy across its assigned tokens is \textbf{bimodal}—the expert handles some tokens easily (low learntropy) and others poorly (high learntropy)—signaling two distinct populations that would benefit from separate treatment. Split procedure:
\begin{enumerate}
    \item Score each expert's tokens by per-token loss.
    \item Test for bimodality (Hartigan's dip test). If bimodal, the expert is a
          split candidate.
    \item Duplicate the expert's LoRA adapter. Each child inherits parent weights
          plus a small random perturbation, starting at \textbf{rank-1}.
    \item Train children on their respective data partitions.
    \item Apply contrastive loss between siblings (Section~\ref{sec:contrastive}).
\end{enumerate}
The split is triggered by an information-theoretic signal, not a schedule.

\subsection{Variable Rank as Specialization Measure}

After splitting, each child begins at rank-1. Rank grows when reconstruction
error exceeds a threshold—the adapter needs more capacity to express what the
data demands. The rank \emph{is} the measurement of specialization depth:
\begin{itemize}
