...
Trunk-18 + Phase 3 (rank 4) & 10 & 0.383$^*$ & 19.98$^*$ & 655K \\
\bottomrule
\end{tabular}
\end{table}
...
The trunk-18 configuration achieves faster differentiation (CosSim 0.771 after
Phase 2 vs.\ 0.840 for trunk-14) with 3$\times$ fewer parameters, despite
having fewer expert layers. In Phase 3, CosSim dropped to 0.383 at rank 4
without rank growth---deeper differentiation than trunk-14 achieved at rank 4
(0.840). This supports the hypothesis that placing experts in the
domain-divergent zone (Section~\ref{sec:divergence}) accelerates
specialization.

No second fork (2$\to$4 experts) has triggered in any configuration. The
bimodality threshold (0.3) has not been reached, suggesting that on generic C4
data, 2-expert systems may be stable equilibria regardless of trunk boundary.
Domain-specific curricula may be needed to trigger deeper forking.
...
of which tokens are routed to them. This reveals \textbf{redundant
representation}: the experts learned different directions in representation
space, but both directions are needed by most tokens. Combined with the layer
divergence analysis (Section~\ref{sec:divergence}), this suggests the experts
were placed in a domain-agnostic zone (layers 14--17) where there was
insufficient domain signal to drive functional specialization.
