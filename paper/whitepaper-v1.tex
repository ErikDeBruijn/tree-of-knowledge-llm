...
The forest is a public good: all adapters are open weights. Incentives
for contribution take three forms. First, \emph{token rewards} compensate
compute costs---contributors receive tokens proportional to the verified
training compute they provided, similar to Bitcoin block rewards. Second,
\emph{reputation} gives contributors governance weight: which domains to
prioritize, which trunk models to support. Third, \emph{self-interest}:
a domain expert who trains an adapter for their own field benefits directly
from the improved model, and their contribution makes the entire forest
stronger. This mirrors the Wikipedia model: the resource is free, and the
incentive to contribute is that you benefit most from the domains you know
best.
