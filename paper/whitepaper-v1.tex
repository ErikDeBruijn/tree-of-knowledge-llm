...
thousands of GPUs, months of compute, and centralized control by a small
number of labs.

This creates three problems:
\begin{enumerate}[nosep]
\item \textbf{Adding knowledge is expensive.} Every new specialty requires
      retraining or fine-tuning the entire model. A hospital that wants
      medical expertise must either pay for a larger model or accept a
      general one.
...
\item \textbf{Knowledge is centralized.} Only organizations with massive
      ...
      consumes what those labs produce.
\end{enumerate}
...
in kilobytes and load in microseconds from NVMe.
...
orders of magnitude. Storing knowledge on flash instead of DRAM reduces
hardware costs and e-waste. The efficiency gains compound: a forest of
10,000 specialists is simultaneously more knowledgeable and cheaper to run
than a single monolithic model trained to cover the same domains.
