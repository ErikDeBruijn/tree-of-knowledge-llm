...
A Hessian analysis reveals that uniform routing is a \emph{saddle point}, not
a basin: the LM loss contains escape directions toward selective routing, but
auxiliary losses and optimization dynamics prevent exploitation. This
suggests the path to causal locality lies in optimization (router learning
rates, auxiliary loss removal) rather than fundamental loss redesign.
\end{abstract}
...
The LM loss landscape contains abundant escape directions from uniform
routing. The optimizer does not exploit them, likely because: (a)~the
load-balancing loss actively penalizes non-uniform routing, counteracting
the curvature signal; (b)~the router learning rate may be too low relative to
the Hessian curvature; or (c)~gradient noise from stochastic batches masks the
directional signal. This refines our earlier conclusion: the training signal
\emph{does} encode routing selectivity, but the optimization dynamics prevent
it from being realized. The constraint is not the loss function itself but the
interaction between the LM loss and auxiliary losses during training.
