...
confirming that the LM loss \emph{does} encode routing selectivity. The path
to causal locality is therefore to replace argmax with a differentiable routing
mechanism such as Gumbel-softmax~\cite{jang2017gumbel} or a straight-through
estimator, allowing the LM loss gradient to flow through the routing decision.
...
However, the Hessian analysis (Section~\ref{sec:hessian}) provides a concrete
path forward: the LM loss \emph{does} encode selective routing directions, but
hard argmax routing blocks the gradient signal. Replacing argmax with
Gumbel-softmax~\cite{jang2017gumbel} would allow the LM loss to directly
train the router toward the saddle point's escape directions.
