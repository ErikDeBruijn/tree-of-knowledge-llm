...
are genuinely distinct achievements. Scaling to Qwen3-8B breaks through:
$M_{ij}$ CV reaches 0.50--0.67, and the expert-augmented model reduces overall
perplexity by 19\% across all domains compared to the base model.
\end{abstract}
...
\begin{table}[h]
\centering
\caption{PPL comparison: base Qwen3-8B vs.\ ToK-8B (with 2 LoRA expert
adapters, rank-4, layers 24--35). 30 samples per domain from C4 validation.}
\label{tab:8b-quality}
\begin{tabular}{@{}lrrr@{}}
\toprule
Domain & Base PPL & ToK PPL & $\Delta$ \\
\midrule
...
\textbf{3.~Causal modularity (achieved at 8B, open at 1.7B).} At 1.7B, the
ablation matrix $M_{ij}$ (Table~\ref{tab:ablation-matrix}) is uniform:
removing either expert degrades all domains equally. The $M_{ij}$ CV plateaus
at $\sim$0.16 across all configurations tested (2--48 experts, 5 loss variants,
4 routing mechanisms). However, scaling to Qwen3-8B breaks through this
ceiling: $M_{ij}$ CV reaches 0.50--0.67, with per-expert ablation damage
varying 3.5--13.5$\times$ across domains
(Section~\ref{sec:8b}, Table~\ref{tab:8b-mij}). The expert-augmented 8B model
also reduces perplexity by 19\% compared to the base
(Table~\ref{tab:8b-quality}), confirming that adapters add knowledge without
...
be active for every token. With diagonal $M_{ij}$, irrelevant experts can
remain on NVMe until needed. Causal locality thus connects all three pillars.

The Hessian analysis (Section~\ref{sec:hessian}) confirms the LM loss
\emph{does} encode selective routing directions. The path forward combines
differentiable routing (Gumbel-softmax~\cite{jang2017gumbel}) with many more,
smaller experts. Multi-furcation (1$\to$N children) could exploit multimodal
learntropy distributions directly, echoing the extreme expert counts that
enable domain specialization in Monet~\cite{monet2025}.

\paragraph{Toward continuous Piagetian adaptation.}
The 4-expert collapse (Section~\ref{sec:8b}) reveals a limitation of
fixed-schedule training: with a global learning rate, early-converging experts
monopolize gradient signal and prevent new experts from activating. We
hypothesize that \emph{learntropy-modulated per-expert learning rates} can
break this deadlock. The learntropy signal---per-token cross-entropy weighted
by routing contribution---naturally distinguishes three regimes:
\begin{itemize}[nosep,leftmargin=1em]
\item \textbf{Low learntropy} (tokens easy for this expert) $\to$ LR
      decreases $\to$ \emph{assimilation mode}: the expert consolidates
      existing representations.
\item \textbf{High learntropy} (tokens hard for this expert) $\to$ LR
      increases $\to$ \emph{accommodation mode}: the expert actively
      restructures to handle new patterns.
\item \textbf{Bimodal learntropy} (two distinct token populations) $\to$
      \emph{split signal}: the expert's schema no longer fits a single
      distribution and should fork.
\end{itemize}
This replaces the discrete freeze/train/split cycle with a continuous
adaptation schedule driven by the model's own difficulty signal---closer to
Piaget's original formulation where the learner, not the teacher, determines
when accommodation is needed. If validated, this would close the gap between
our framework's theoretical design (learntropy-driven tree growth) and its
practical training dynamics.

\paragraph{Three dimensions of the learntropy control signal.}
The learntropy-modulated LR is only one of three control dimensions that the
same signal could drive simultaneously:

\emph{(1)~Functional divergence and fork boundaries.}
Our CKA/fisher divergence analysis (Section~\ref{sec:8b}) uses
\emph{domain} categories (medical, code, legal). However, functional
divergence---syntax vs.\ semantics, operators vs.\ operands, abstraction
vs.\ instantiation---may show a different profile, possibly peaking at
earlier layers. If functional divergence ramps at layer~12 while domain
divergence ramps at layer~24, the optimal fork boundary depends on
\emph{what} the expert specializes in. This motivates variable-depth
forking: functional experts branch early (broad but expensive), domain
experts branch late (narrow but cheap).

\emph{(2)~Information-theoretic ceiling.}
The 2-expert equilibrium at 8B has an information-theoretic
explanation: rank-4 LoRA adapters operate in a 4-dimensional subspace of a
4096-dimensional hidden space ($<$0.1\% of available directions). The
residual error that experts can correct is inherently low-rank, and 2
principal components may exhaust the useful signal in that subspace. Three
knobs can expand this: earlier forking (more divergent representations to
correct), higher rank (more dimensions per expert), or learntropy-driven
adaptation (letting the model allocate capacity where needed). These are
orthogonal and likely complementary.

\emph{(3)~Meta-learned training policy.}
The logical endpoint is a \emph{training controller} that observes learntropy
statistics (mean, variance, kurtosis per expert per layer) and takes actions:
adjust LR, trigger splits, increase rank, modify loss weights. This is
the state/action space of a reinforcement learning problem, where reward is
$M_{ij}$ CV improvement per training episode. The 25+ experiment trajectories
in this paper provide offline training data for such a controller.
