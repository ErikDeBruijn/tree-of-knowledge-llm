...
Domain selectivity scales as a shallow power law with expert count
($\text{CV} \propto N^{0.16}$), predicting that 50--600 experts are needed
for meaningful causal locality---consistent with recent extreme-expert
architectures.
\end{abstract}
...
CV$=$0.162---identical to 16 experts. The M$_{ij}$ CV appears to \emph{ceiling}
at $\sim$0.16 for this base model, regardless of expert count. This suggests
the bottleneck is not expert count but the base model's representational
capacity for domain-distinguishable features at 1.7B parameters.
