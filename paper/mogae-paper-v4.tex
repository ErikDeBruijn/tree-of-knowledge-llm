...
are the same dimension. The results are positive on two of three fronts:
...
expert degrades all domains equally. Three independent routing analyses
confirm uniform domain routing (49--51\% split). Weight orthogonality
(CosSim $<$0.3) is necessary but not sufficient for modularity.

Our central methodological conclusion: \textbf{hot-pluggability requires
causal locality, not orthogonality.} The correct metric is whether expert
removal causes \emph{selective} damage, not whether expert weights occupy
different subspaces. The Standing Committee~\cite{standing2026} and
DeepSeekMoE~\cite{deepseek2024moe} results suggest that shared + routed
architectures and top-$K$ routing ($K > 1$) are likely prerequisites for
achieving causal locality at small scale.
