...
These questions were originally framed for a multi-level tree with variable
ranks. Our experiments produced only 2-expert systems at fixed rank, so we
reformulate them in light of actual results.
...
% TODO: Fill with rank sweep results when available.

\placeholder{Table: Rank sweep results. For each rank (1,2,4,8,16,32):
trainable params, PPL after 5K steps, PPL gain, RCR. Expected: diminishing
returns at higher ranks (lens$\to$crystal transition).}
...
\begin{enumerate}
    \item \textbf{Weight differentiation does not imply functional
          ...
          mechanisms beyond contrastive loss, likely including top-$K$ routing
          or modularity-aware training objectives.
