...
% ============================================================================
\begin{abstract}
We investigate how to separate a language model's \emph{fluid intelligence}
(reasoning, syntax, attention patterns) from its \emph{crystallized
intelligence} (domain-specific knowledge) using variable-rank LoRA adapters on
FFN layers. Our central finding is that \textbf{weight-space differentiation
does not imply functional modularity}: contrastive loss reliably produces
orthogonal expert weights (cosine similarity $<$0.3), but routing analysis
shows both experts contribute uniformly across all domains. This holds
regardless of fork depth (layers 14 vs.\ 18) or adapter rank (4 vs.\ 32).

We ground this finding in systematic experiments on Qwen3-1.7B: a layer
divergence analysis identifies where domain signals emerge (layer 18, not 14
as initially assumed); a LoRA magnitude analysis reveals experts contribute
25\% of FFN output but with $<$5\% routing selectivity; and three independent
routing analyses confirm uniform domain routing. These results align with
recent MoE interpretability work showing that functional specialization
(structure vs.\ content) emerges before domain specialization, and that
top-$K$ routing ($K > 1$) may be required for multi-axis
specialization~\cite{standing2026, domaindriver2026}.

We propose that \emph{causal locality}---selective ablation damage rather than
weight orthogonality---is the correct metric for modularity, and formalize
rank growth as Piagetiaan accommodation: capacity expands only when residual
error cannot be assimilated within the current update-subspace.
\end{abstract}
...
relative to the parent representation. We formalize rank growth as Piagetiaan
...
the error requires capacity the expert does not have---rank should grow. This
replaces hand-tuned reconstruction-error thresholds with a principled criterion
grounded in subspace geometry.
...
are the same dimension. Our most important finding is negative:
\textbf{weight-space orthogonality is not modularity.}

Contrastive LoRA forking reliably produces orthogonal expert weights
(CosSim $<$0.3), and layer divergence analysis correctly identifies where
domain signals emerge (layer 18 in Qwen3-1.7B). But three independent routing
analyses across different configurations all show uniform domain routing
(49--51\% split) and uniform ablation damage. The experts learned different
\emph{directions} in representation space without learning different
\emph{functions}.

This leads to our central methodological conclusion:
\textbf{hot-pluggability requires causal locality, not orthogonality.} The
correct metric for modularity is the ablation matrix $M_{ij} = \Delta\text{loss
on task } i \text{ when expert } j \text{ is removed}$. A modular system has
near-diagonal $M$; our system has uniform $M$. Cosine similarity, subspace
separation, and weight orthogonality are hygiene metrics, not modularity
metrics.

We formalize rank growth as Piagetiaan \emph{accommodation}
(Eq.~\ref{eq:accommodation}): the minimum rank needed is the dimensionality
of the residual structure that cannot be assimilated within the parent's
update-subspace. This replaces heuristic thresholds with a principled,
measurable criterion.

The architecture itself---compact, variable-rank LoRA adapters on FFN layers,
trainable on commodity hardware---remains sound. Recent independent work
confirms the design: HydraLoRA~\cite{hydralora2024} shares the shared-A +
routed-B structure, and MoLoRA~\cite{molora2026} validates per-token adapter
routing. The open problem is achieving causal locality: producing experts whose
ablation matrix is diagonal, not uniform. The Standing
Committee~\cite{standing2026} and DeepSeekMoE~\cite{deepseek2024moe} results
suggest that shared + routed expert architectures and top-$K$ routing ($K > 1$)
