...
partitions, but a teacher-driven niche curriculum achieves CV$=$1.07 on
ZPD-defined token populations---the first level-3 (causal locality) result.
Causal locality requires not domain labels but a \emph{learnable vs.\
already-known} partition, identifiable through a teacher--student gap.
\end{abstract}
...
C4's data imbalance. However, a \textbf{teacher-driven niche curriculum}
using token-level ZPD scores (Section~\ref{sec:zpd}) achieves the first
level-3 result: $M_{ij}$ CV$=$1.07 on niche vs.\ general token populations,
with 92\% of high-ZPD tokens routing to the niche expert. The teacher
(Qwen3-30B) identifies tokens where the student struggles but the teacher
succeeds; these niche tokens define the expert's ownership. This suggests
that causal locality is achievable when the training curriculum explicitly
separates \emph{learnable} from \emph{already-known} token populations---a
Vygotskyan rather than domain-taxonomic partition. A top-2 routing variant
...
CV$=$0.162---identical to 16 experts. The M$_{ij}$ CV appears to \emph{plateau} at $\sim$0.16 for this base
model across the range tested (2--48 experts). While we cannot exclude
optimization difficulties at higher expert counts, this suggests the
bottleneck may be the base model's representational capacity for
domain-distinguishable features at 1.7B parameters.
