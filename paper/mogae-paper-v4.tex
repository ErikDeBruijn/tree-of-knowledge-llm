...
individually indispensable, but not independently removable. We show that
\emph{causal locality} (selective ablation damage) rather than weight
orthogonality is the correct metric for modularity, and that top-$K$ routing
($K > 1$) may be a prerequisite for domain-level specialization at this scale.

We formalize rank growth as Piagetiaan accommodation: capacity expands when
residual error cannot be assimilated within the current update-subspace.
\end{abstract}
