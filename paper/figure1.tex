% Figure 1: Tree of Knowledge — includable via \input{figure1.tex}
...
% === Size annotations along left branches (showing adapter sizes) ===
...
\end{tikzpicture}
\caption{\textbf{Tree of Knowledge architecture.} A single pretrained model (the \emph{Fluid Core}, layers 1--14) is shared across all tasks. At successive network depths, lightweight LoRA adapters branch into increasingly specialized skills. Shallow forks (rank-1, 4\,KB) capture broad distinctions (e.g., code vs.\ domain knowledge); deeper forks (rank-16, 256\,KB) encode fine-grained expertise. The storage hierarchy mirrors the access pattern: the shared trunk stays resident in VRAM, frequently-used branches are cached, and rare specializations are loaded from flash on demand. An entire tree of 10{,}000 specializations fits in ${\sim}$3\,GB --- the same memory as one additional model.}
\label{fig:tree-of-knowledge}
\end{figure*}
