...
- Two-adapter simultaneous gating (cycles 4-5, 2026-03-30): Independent sigmoid: cuisine gate leaks (0.489 on BBC). Softmax normalization fixes leakage (cuisine gate on BBC: 0.489→0.181, on generic: 0.458→0.377) and cuisine PPL from +6.8%→+1.8%. But softmax dampens primary gate (BBC gate 0.786→0.661, below 0.70 threshold) and BBC PPL still +4.7%. Trade-off: normalization fixes cross-gate interference but reduces gate amplitude. PLAUSIBLE — softmax is a significant improvement but not clean enough for full SUPPORTED.
