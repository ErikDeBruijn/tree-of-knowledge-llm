...
- A2 contrastive gate: SUPPORTED (2026-04-03, 2 seeds). Mean selectivity +0.968 (seeds: +0.961, +0.974). Generation IMPROVES from adapter-only 35% to gated 45-50% syntax — gate suppresses adapter on generic text where it would be destructive. Contrastive loss (-log(gate(domain)) - log(1-gate(generic))) solves the gate training problem.
