...
- A2 contrastive gate: WORKS (2026-04-03, 1 seed). Selectivity +0.961 (domain 0.98, generic 0.02) WITHOUT generation degradation (syntax 40-50%). Contrastive loss (-log(gate(domain)) - log(1-gate(generic))) gives gate direct discriminative signal instead of relying on LM loss. Solves the gate training problem.
