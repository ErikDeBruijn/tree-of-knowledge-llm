...
Last updated: 2026-05-30 (corrections below). Prior substantive update: 2026-04-10 (overnight: ruby_realworld expert trained + eval'd — PPL ≠ correctness)

> **Read alongside `paper/mogae-paper-v6.tex` — the most current honest source.** Two items below are superseded by later work:
> 1. **Code-capability "degradation"** (the `ruby_realworld` / `expert_v17` OBSERVED bullets) was a property of *LM-loss* gate training, which collapses generation quality. The **contrastive gate loss** (paper §E8) reverses it: sandboxed-execution correctness goes Ruby **20%→70%**, Python **70%→90%**. The narrower caveat still holds: PPL improvement does *not* predict functional correctness — measure with execution, not PPL.
> 2. **Autonomous idle-time training** is no longer "blocked / untested on real GPU" — it runs in the server. See the new OBSERVED bullet directly below and `GAPS.md` Gap 3.

## Confidence classes (per CHARTER)

### OBSERVED (logs, artifacts, measurements)
- **Autonomous idle-time training loop runs in the server (live on real GPU, 2026-04-03)**: The scheduler (`grove_server/engine/scheduler.py`) serves queued inference with priority and trains only in idle time — mode switch is a hook swap, no model reload. GPU work is serialized through a single-worker executor, gated by an idle cooldown and a duty-cycle training budget that yields to inference and to other GPU processes. Wired into `GroveDaemon`, which launches it via `asyncio.create_task(self.scheduler.run())` (`grove_server/daemon.py`). Full cycle: adapter (LoRA+) → contrastive gate → eval → deploy → split. First live run trained 293 steps in ~30s (commit `8f9aab1`, which modified `grove_server/` internals — i.e. the server's own loop, not an external script). Caveat: single dev run, not a repeated benchmark. **Supersedes `GAPS.md` Gap 3, which was frozen 2026-04-02 — one day before this went live.**
