...
| F11 | Per-token expert attribution (layer-gate heatmap) | Must |
| F12 | Expert selection checkboxes in attribution playground | Must |
| F13 | Response-level attribution summary | Must |
| F14 | Input attention visualization (which input tokens influenced output) | Should |
| F15 | Functional experts (style/register, not just domain) | Should |
| F16 | Auto-load experts from directory at startup | Should |
| F17 | OpenAPI documentation at /docs | Must |
...
### Phase 0+ Features (2026-04-02, in progress)

**Implemented:**
- Fast pipeline: GraphableDecodeStep + StaticKVCache (64 tok/s, up from 12 tok/s)
- FP8 inference path with fixed activation scale for Blackwell
- Per-token expert attribution: `/v1/completions` returns layer-gate activations per token
- Attribution Playground: hover any token for 36-layer gate heatmap
- Chat Playground: streaming chat at `/playground/chat`
- Expert checkboxes: select which experts are active for completion
- Response-level attribution summary (average gate heatmap across all tokens)
- Navigation bar between Attribution, Chat, and Dashboard
- OpenAPI docs at `/docs` (Swagger UI, auto-generated by FastAPI)
- Metrics flow: streaming inference records tok/s and request count to dashboard
- `.pt` format expert loader (training engine checkpoint format)

**Planned (from user requests):**
- Multi-expert routing: multiple experts active simultaneously with softmax gates
- Functional experts: style/register adapters (not just domain), e.g. "formal" adapter shared by medical + legal
- Input attention visualization: show which input tokens each output token attends to
- Second expert (legal/formal) for cross-domain attribution demo
- Auto-load experts from `--experts-dir` at startup
- CUDA graph decode (disabled: needs tensorized cache position)
